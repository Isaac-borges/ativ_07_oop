...
class Banco {
    ...
    renderJuros(numero_conta: string) : void{
        let conta_procurada: Conta = this.consultarConta(numero_conta);

        if (conta_procurada instanceof Poupanca) {
            conta_procurada.renderJuros()
        }
    }
}
